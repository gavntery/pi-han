...
function OptimizeSplineCoordinates( width, height, ref_A_B, ref_B_A, tolerance )
{
   ...
   this.Optimize = function()
   {
      ...
      console.writeln( "<end><cbr>Optimizing spline-based distortion model..." );

      console.writeln( format( "%d control points, maximum error: %.3f px", this.cpA.length, maxErr ) );
      var newPoints = true;
      while ( newPoints && maxErr > this.tolerance && this.cpA.length < 2000 )
      {
         newPoints = false;
         var newBlocks = [];
         maxErr = 0;
         ...
         {
            var block = blocks[i];
            if ( i > 50 || block.err < this.tolerance )
